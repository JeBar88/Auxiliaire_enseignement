...
##### Fonction utile #####
# dPoTei <- function(m1, m2, lam1, lam2, a0){
...
#   f <- matrix(0, ncol = length(y), nrow = length(y))
#   for (i in x) {
#     for (j in y) {
...
# Façon simple, mais pas la plus rapide, si domaine grand utiliser for
...
sum(outer(pmin(m1, 7), m2 * I(m2 <= 6), '*')*fm12) # 5.767054
